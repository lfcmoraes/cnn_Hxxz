...
#resolve monitor problem running on cluster
...
#plotting data set information, GS energy density and magnatization
a = np.array(row[:,3]).astype(np.float) #h
b = np.array(row[:,2]).astype(np.float) #E_GS/l
c = np.array(row[:,1]).astype(np.float) #mag_GS
...
#magnetization os the ground state
plt.plot(a,c)
